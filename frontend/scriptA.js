...
window.renderCart = function() {
    ...
    window.cartItems.forEach((item, index) => {
        html += '<li style="display: flex; justify-content: space-between; margin-bottom: 10px; border-bottom: 1px solid rgba(255, 255, 255, 0.1); padding-bottom: 8px;">' +
                    ...
                    '<span style="color: #3399ff; font-weight: 700;">₹ ' + item.price.toLocaleString('en-IN') + '</span>' +
                 '</li>';
        total += item.price;
    });
    
    html += '</ul>';
    html += '<div style="display: flex; justify-content: space-between; font-weight: bold; font-size: 1.1rem; color: #ffffff; margin-top: 15px; border-top: 2px solid rgba(51, 153, 255, 0.3); padding-top: 12px;">' +
                '<span>Total:</span>' +
                '<span style="color: #3399ff; font-weight: 800;">₹ ' + total.toLocaleString('en-IN') + '</span>' +
             '</div>';
